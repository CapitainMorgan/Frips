...
import { TextField,InputAdornment,Dialog } from "@material-ui/core";
...
import { StateClothes } from "../staticItems/staticItemName";
...
const StateForm = ({field,form,mobile}) =>{
    ...
    const renderedStateClothes = SizeForm.itemconditionInfo.map(item =>{
        ...
        return (
            <li  style={{ listStyleType:"none",width:"100%"}}  onClick={()=>
            {
    ...
    return(
        ...
        <Dialog open={Boolean(anchorEl)}>
            <Box className={classes.Dialog} display="flex" flexDirection="column" >
                ...
                <Box  style={{backgroundColor:"white",overflowY:"auto"}} width={"100%"}>
                        {renderedStateClothes}
                </Box>
