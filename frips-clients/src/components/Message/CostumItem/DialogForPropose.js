import { Box,makeStyles,ClickAwayListener,Dialog,Typography,IconButton,Button} from '@material-ui/core'
import React,{useState,useMemo,useEffect} from 'react'
import CloseIcon from '@material-ui/icons/Close';
import { connect,useDispatch,useSelector } from 'react-redux';
import { Card,CardActionArea,CardActions,Divider,CardHeader,Avatar } from '@material-ui/core';
import { addFavorite, handleClickAwayPropose, handleClickAwaySecondPage, handleClickPropose, handleClickSecondPage } from '../../../actions';
    ...
    return state.map((item,index)=>{
        return(
            ...
            <Card className={classes.BoxOneItem}> 
                    ...
                    <img alt={`/images/${state[index].id}/${state[index].image[0].image}`} src={`/images/${state[index].id}/${state[index].image[0].image}`} style={{width:"100%",height:"100%",objectFit:"cover"}} />
...
const ListForPropose = ({id,items,itemFromId,AnchorEl,loading,loaded,secondPage,socket}) => {
    ...
    <ClickAwayListener onClickAway={handleClickAway}>
        ...
        <Box >
          <Button  type="outlined" color="primary" onClick={handleClick}>
                            ...
                            {!secondPage && AnchorEl? <Dialog transitionDuration={0} open={Boolean(AnchorEl && !secondPage)}>
                            ...
                            <Box className={classes.Dialog} display="flex" flexDirection="column" >
                                ...
                                <SecondPageDialog id={id} socket={socket} item={item} handleClickAway={handleClickAway}  classes={classes} />                 
