        ...
        return(
            ...
            <Card className={classes.boxShadow}> 
                ...
                />
                    ...
                    <img src={`http://localhost:5000/images/${state[index].image[0].id_Item}/${state[index].image[0].image}`} style={{width:"100%",height:"100%",objectFit:"cover"}} />
...
const MyItems = ({items,loading}) => {
    ...
    if(items.length <1 && loading){
        return (
          <Box height="100vh" width="100%" style={{backgroundColor:"#F5f5f3"}} >
